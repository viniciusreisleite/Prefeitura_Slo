...
with sync_playwright() as p:
    ...
    try:
        ...
        # Clica no primeiro Reel disponível para disparar a reprodução e captura do link
        ...
        if first_reel.count() > 0:
            ...
            # Tenta pegar a tag de vídeo caso a requisição de rede não tenha capturado
            if not video_url:
                video_elem = page.locator("video").first
                if video_elem.count() > 0:
                    video_url = video_elem.get_attribute("src")
        else:
            # Se não houver aba de Reels, testa o primeiro post do feed
...
if video_url:
    print(f"Vídeo capturado com sucesso! Baixando mídia...")
    video_file = f"{post_id}.mp4"
    
    # Baixa usando os cookies de sessão
    headers = {
